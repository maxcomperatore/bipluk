...
def init_db():
    ...
    try:
        ...
        # Create rate_limits table for atomic distributed rate limiting across serverless instances
        cursor.execute("""
        ...
        # Create user_sessions table for stateful session validation and instant revocation
        cursor.execute("""
...
def add_to_unsubscribed(email: str):
    ...
    if not clean_email or "{{" in clean_email or "%7b" in clean_email or "@" not in clean_email or "." not in clean_email:
        return
    try:
        conn = get_db_connection()
        try:
...
def check_and_increment_rate_limit(key: str, max_requests: int, window_seconds: int) -> tuple[bool, int]:
    """
    Atomic distributed rate limiter backed by PostgreSQL.
    Returns (is_allowed, retry_after_seconds).
    Uses row-level locking on conflict to ensure atomic increment across all serverless Lambdas/ASGI workers.
    """
    conn = get_db_connection(max_retries=1, retry_delay=0.2)
    try:
        cursor = conn.cursor()

        # Opportunistic cleanup of expired windows (approx 1 in 50 calls)
        if random.random() < 0.02:
            try:
                cursor.execute("DELETE FROM rate_limits WHERE window_start < NOW() - INTERVAL '1 hour';")
                conn.commit()
            except Exception:
                pass
        ...
        SET count = CASE
            WHEN rate_limits.window_start < NOW() - (%s || ' seconds')::INTERVAL THEN 1
            ELSE rate_limits.count + 1
        END,
        window_start = CASE
            WHEN rate_limits.window_start < NOW() - (%s || ' seconds')::INTERVAL THEN NOW()
            ELSE rate_limits.window_start
        END
        RETURNING count, GREATEST(1, CEIL(EXTRACT(EPOCH FROM (window_start + (%s || ' seconds')::INTERVAL - NOW()))))::INT AS retry_after;
        """
        cursor.execute(query, (key, str(window_seconds), str(window_seconds), str(window_seconds)))
        row = cursor.fetchone()
        conn.commit()
        if row:
            curr_count, retry_after = row
            retry_after = max(1, retry_after if retry_after is not None else window_seconds)
            return (curr_count <= max_requests, retry_after)
        return (True, 0)
    finally:
        try:
            conn.close()
...
def create_user_session(user_id: int, session_token: str) -> bool:
    """Register a new active session token for a user in PostgreSQL."""
    try:
        conn = get_db_connection()
        try:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO user_sessions (user_id, session_token) VALUES (%s, %s) ON CONFLICT (session_token) DO NOTHING;",
...
def get_user_by_session(email: str, session_token: str) -> dict | None:
    """Validate that a session token exists in PostgreSQL and matches the user email."""
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                ...
                WHERE s.session_token = %s AND LOWER(u.email) = LOWER(%s);
                """,
                (session_token, email.strip())
            )
            row = cursor.fetchone()
            return dict(row) if row else None
        finally:
        ...
        print(f"Database error in get_user_by_session: {e}")
        return None


def delete_user_session(session_token: str) -> bool:
    """Delete a single session token on logout."""
    ...
    try:
        ...
        print(f"Database error in delete_user_session: {e}")
        return False


def delete_all_user_sessions(user_id: int) -> bool:
    """Revoke all sessions for a user (called on password reset or account suspension)."""
    if not user_id:
        return False
    try:
        conn = get_db_connection()
        ...
        print(f"Database error in delete_all_user_sessions: {e}")
        return False
