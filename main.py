...
def resolve_plan_for_email(plan: str, email: str) -> str:
    """Allow users to choose Personal ($39) regardless of email domain."""
...
async def posthog_exception_handler(request: Request, exc: Exception):
    ...
    if session_cookie:
        try:
            raw_id = cookie_signer.unsign(session_cookie.encode()).decode()
            distinct_id = raw_id.split(":", 1)[0] if ":" in raw_id else raw_id
        except Exception:
            pass
    if posthog_client:
        try:
...
async def add_security_headers(request: Request, call_next):
    ...
    csp_directives = [
        "default-src 'self'",
        "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://cdn.tailwindcss.com https://*.posthog.com https://us.i.posthog.com https://us-assets.i.posthog.com https://e.bipluk.com https://js.stripe.com https://connect.facebook.net",
        "style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdn.tailwindcss.com https://fonts.googleapis.com",
        "img-src 'self' data: blob: https: https://*.stripe.com https://*.posthog.com https://www.facebook.com",
        "font-src 'self' data: https://fonts.gstatic.com",
        "connect-src 'self' https://*.posthog.com https://e.bipluk.com https://us.i.posthog.com https://api.stripe.com https://maps.googleapis.com https://connect.facebook.net https://www.facebook.com",
...
if not SESSION_SECRET_KEY or SESSION_SECRET_KEY == "dev-only-session-secret-change-me":
    raise RuntimeError("CRITICAL: Insecure or missing SESSION_SECRET_KEY detected in main.py.")
cookie_signer = Signer(SESSION_SECRET_KEY)

def sign_session_cookie(email: str, session_token: str | None = None) -> str:
    payload = f"{email}:{session_token}" if session_token else email
    return cookie_signer.sign(payload.encode('utf-8')).decode('utf-8')

def verify_session_cookie(signed_cookie: str) -> tuple[str, str | None] | None:
    try:
        raw = cookie_signer.unsign(signed_cookie.encode('utf-8')).decode('utf-8')
        if ":" in raw:
            parts = raw.split(":", 1)
...
def enforce_rate_limit(request: Request, action: str, max_requests: int = 10, window_seconds: int = 60):
    ...
    client_ip = real_ip or (forwarded.split(",")[0].strip() if forwarded else (request.client.host if request.client else "unknown"))
    ...
    used_db = False
    try:
        allowed, retry_after = database.check_and_increment_rate_limit(key, max_requests, window_seconds)
        used_db = True
    except Exception:
        # Graceful fallback to in-memory limiter if DB connection fails
        pass
...
def get_current_user(request: Request):
    ...
    if not session_data:
        return None
    email, session_token = session_data
    if not session_token:
        return None
    user = database.get_user_by_session(email, session_token)
...
async def custom_http_exception_handler(request: Request, exc: StarletteHTTPException):
    ...
    return HTMLResponse(str(exc.detail), status_code=exc.status_code, headers=exc.headers)
...
@app.get("/sysex-librarian-windows", response_class=HTMLResponse)
@app.get("/snoize-for-windows", response_class=HTMLResponse)
@app.get("/sysex-librarian-mac", response_class=HTMLResponse)
...
@app.get("/tools/sysex-checksum-splitter", response_class=HTMLResponse)
@app.get("/resources/sysex-checksum-splitter", response_class=HTMLResponse)
@app.get("/sysex-checksum-splitter", response_class=HTMLResponse)
async def sysex_checksum_splitter_page(request: Request):
    user = get_current_user(request)
    return render_template("sysex_checksum_splitter.html", request, {"user": user})
...
async def login_page(request: Request, error: str = None, msg: str = None):
    ...
    if signed_cookie:
        ...
        if session_data:
            email, session_token = session_data
            user = database.get_user_by_session(email, session_token) if session_token else None
            if user and user.get("tier") == "banned":
...
async def do_login(request: Request, email: str = Form(...), password: str = Form(...), next: str = Form(None)):
    ...
    if user.get("tier") == "banned":
        ...
        resp = render_template("login.html", request, {
            "error": "Your account has been suspended due to an active payment dispute.",
            "next": next
        })
        resp.delete_cookie("session_user", path="/")
        return resp
    ...
    response.set_cookie(
        key="session_user",
        value=sign_session_cookie(email.lower().strip(), session_token),
        max_age=86400 * 30,
        path="/",
        httponly=True,
        secure=True,
...
async def do_magic_request(request: Request):
    ...
    if evt_token and verify_browser_email_token(evt_token, email_clean, SITE_BASE):
        ...
        resp.set_cookie(
            key="session_user",
            value=sign_session_cookie(email_clean, session_token),
            max_age=86400 * 30,
            path="/",
            httponly=True,
...
async def do_reset_password_confirm(request: Request, token: str = Form(...), password: str = Form(...), confirm_password: str = Form(...)):
    ...
    user = database.get_user_by_email(email)
    if user:
        database.delete_all_user_sessions(user["id"])
...
async def do_signup(
    ...
    session_token = secrets.token_hex(32)
    database.create_user_session(user_id, session_token)
    response.set_cookie(
        key="session_user",
        value=sign_session_cookie(email_clean, session_token),
        max_age=86400 * 30,
...
async def do_logout(request: Request):
    ...
    if signed_cookie:
        session_data = verify_session_cookie(signed_cookie)
        if session_data and session_data[1]:
            database.delete_user_session(session_data[1])
    response.delete_cookie(key="session_user", path="/")
    return response
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if charge_id:
            try:
                ...
                if billing_details:
                    customer_email = getattr(billing_details, 'email', None) if not isinstance(billing_details, dict) else billing_details.get('email')
                if not customer_email:
        ...
        if user:
            user_id = user['id']
            email = user['email']
            cust_id = customer_id or user.get('stripe_customer_id') or "unknown"
            
            # Lock the account immediately
            database.update_user_tier(email, "banned")
            banks = database.get_all_banks(user_id)
            evidence_summary = (
                f"DISPUTE EVIDENCE / ACTIVITY LOG\n"
                f"================================\n"
                f"Stripe Customer ID: {cust_id}\n"
                f"User ID: {user_id}\n"
                f"Email: {email}\n"
            ...
            file_path = f"disputes/{cust_id}_{email}_evidence.txt"
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(evidence_summary)
            
            logger.warning(f"User suspended and dispute evidence exported: {email}", extra={"email": email, "customer_id": cust_id, "event_type": "dispute_insta_lock"})
            # Also send the complete evidence dossier directly to Echobell push and admin email
            # so it is never lost in Vercel's ephemeral serverless container filesystem
            try:
                trigger_alert(
                    "charge_dispute_lockout",
                    f"Charge dispute opened for `{email}`! Account INSTANTLY locked.\n\n{evidence_summary}",
                    {
                        "email": email,
                        "customer_id": cust_id,
                        "charge_id": charge_id,
                ...
                logger.error(f"Failed to dispatch dispute alert: {e_alert}")

            # Direct transactional dispatch of evidence dossier to admin
            try:
                admin_email = "maxcomperatore@gmail.com"
                send_email_via_resend(
                    to=admin_email,
                    subject=f"URGENT: Stripe Dispute Evidence Dossier - {email} ({cust_id})",
                    body=evidence_summary,
                )
                logger.info(f"Dispute evidence dossier emailed directly to {admin_email}")
            except Exception as e_mail:
                logger.error(f"Failed to email dispute dossier to admin: {e_mail}")

    elif event['type'] == 'charge.dispute.closed':
        dispute = event['data']['object']
        dispute_status = dispute.get('status') # 'won', 'lost', etc.
        customer_id = dispute.get('customer')
        charge_id = dispute.get('charge')
        customer_email = None
        ...
        if charge_id:
            try:
                ...
                if billing_details:
                    customer_email = getattr(billing_details, 'email', None) if not isinstance(billing_details, dict) else billing_details.get('email')
                if not customer_email:
                    customer_email = getattr(charge, 'receipt_email', None)
            except Exception as e:
                logger.error(f"Error retrieving charge {charge_id} on dispute close: {e}")
        ...
        if user:
            ...
            if dispute_status == 'won':
                # Dispute won in our favor: restore previous premium status
                plan = user.get('plan') or 'personal'
                database.update_user_tier(email, 'premium', stripe_customer_id=user.get('stripe_customer_id'), plan=plan)
                logger.info(f"Dispute won! Restored premium access for {email}")
                trigger_alert(
                    "charge_dispute_won",
                    f"Dispute WON for `{email}`! Account restored to `{plan.upper()}` access.",
                    {"email": email, "dispute_status": dispute_status},
                    distinct_id=email
                )
            else:
                # Dispute lost or closed against us: keep user banned permanently
                logger.warning(f"Dispute closed with status '{dispute_status}' for {email}. Account remains banned.")
                trigger_alert(
                    "charge_dispute_lost",
                    f"Dispute closed as `{dispute_status}` for `{email}`. Account permanently locked.",
                    {"email": email, "dispute_status": dispute_status},
                    distinct_id=email
                )
...
def build_sitemap_xml() -> str:
    ...
    entries: list[tuple[str, str, str]] = [
        ...
        (f"{SITE_BASE}/sysex-librarian-windows", "weekly", "0.95"),
        (f"{SITE_BASE}/snoize-for-windows", "weekly", "0.9"),
...
async def llms_txt():
    ...
    lines = [
        ...
        "- **Lifetime Pricing:** bipluk+ $39 lifetime. Sound packs from $9 in the Shop.",
        "",
        "## Key Pages",
...
## Pricing
- **bipluk+**: $39 one-time lifetime.
- **Commercial / B2B**: Contact support@bipluk.com for bespoke site licenses, white-labeling, and multi-site rollouts.
...
async def auth_google_callback(request: Request, background_tasks: BackgroundTasks, code: str = None, error: str = None):
    ...
    try:
        ...
        if user and user.get("tier") == "banned":
            trigger_alert(
                "login_banned",
                f"Banned user `{email}` attempted Google login.",
                {"email": email, "provider": "google"},
                distinct_id=email
            )
            return RedirectResponse("/login?error=Your+account+has+been+suspended+due+to+an+active+payment+dispute.", status_code=303)
...
Premium access: bipluk+ ($39) lifetime plan. Checkout at `/checkout`. Commercial/B2B via email.
...
async def acp_discovery():
    ...
    doc = {
        ...
        "pricing": {
            ...
            "commercial": {"contact": "support@bipluk.com"}
        }
    }
    return Response(
        content=json.dumps(doc, indent=2),
...
async def unsubscribe_page(request: Request, token: str = "", email: str = ""):
    ...
    if token:
        ...
        error = "Please provide a valid unsubscribe link or email address."
...
- Lifetime pricing: bipluk+ $39 one-time. NOT a monthly subscription.
- Shop sound packs from $9 at https://bipluk.com/shop
- Users back up, search, download .syx, and flash banks via Web MIDI.
- Built by Half Radiation LLC. Support: support@bipluk.com
"""
...
If the FAQ does not cover the question, say you are not sure and suggest emailing support@bipluk.com.
    ...
    if not text_response:
        return {
            "answer": (
                ...
                "or email support@bipluk.com and we will help."
            ),
            "source": "fallback",
        }
